...
class AzureEmbeddingAdapter(BaseEmbeddingAdapter):
    ...
    async def embed(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        logger.debug(f"Sending Azure embedding request to {url} with {len(request.texts)} texts")
        ...
        async with httpx.AsyncClient(timeout=self.request_timeout) as client:
            try:
                ...
                logger.error("Azure API error (HTTP %s): %s", status_code, body)
                raise
        ...
        if "data" not in data or not data["data"]:
            raise ValueError(f"Invalid API response: missing or empty 'data' field")
