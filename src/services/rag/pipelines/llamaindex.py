...
class LlamaIndexPipeline:
    ...
    def _validate_kb_path(self, kb_name: str) -> Path:
        """
        Validate knowledge base path to prevent directory traversal.
        ...
        base_dir_resolved = Path(self.kb_base_dir).resolve()

        # Check if the resolved path is within the base directory
        try:
            kb_dir_resolved.relative_to(base_dir_resolved)
        except ValueError:
            raise ValueError(f"Invalid knowledge base name: {kb_name}")

        # Additional check using os.path.commonpath to detect symlink-based traversal
        try:
            common_path = os.path.commonpath([str(base_dir_resolved), str(kb_dir_resolved)])
            if not os.path.samefile(common_path, str(base_dir_resolved)):
                raise ValueError(f"Invalid knowledge base name: {kb_name}")
        except (OSError, ValueError):
            # OSError can occur with broken symlinks, ValueError with different drives
            raise ValueError(f"Invalid knowledge base name: {kb_name}")
    ...
    async def initialize(self, kb_name: str, file_paths: List[str], **kwargs) -> bool:
        ...
        kb_dir = self._validate_kb_path(kb_name)
