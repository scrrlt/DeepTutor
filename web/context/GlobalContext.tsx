...
export function GlobalProvider({ children }: { children: React.ReactNode }) {
  ...
  const [sidebarDescription, setSidebarDescriptionState] = useState<string>(DEFAULT_DESCRIPTION)
  const [sidebarNavOrder, setSidebarNavOrderState] = useState<SidebarNavOrder>(DEFAULT_NAV_ORDER)
  ...
  const saveSolverState = useCallback(
    debounce((state: SolverState) => {
      ...
      const toSave = persistState(state, EXCLUDE_FIELDS.SOLVER as unknown as (keyof SolverState)[])
  ...
  const saveChatState = useCallback(
    debounce((state: ChatState) => {
      if (!isHydrated.current) return
      const toSave = persistState(state, EXCLUDE_FIELDS.CHAT as unknown as (keyof ChatState)[])
  ...
  useEffect(() => {
    ...
    const persistedSolver = loadFromStorage<Partial<SolverState>>(STORAGE_KEYS.SOLVER_STATE, {})
    ...
    const persistedIdeaGen = loadFromStorage<Partial<IdeaGenState>>(STORAGE_KEYS.IDEAGEN_STATE, {})
    const persistedChat = loadFromStorage<Partial<ChatState>>(STORAGE_KEYS.CHAT_STATE, {})
