...
export function IdeaGenProvider({ children }: { children: React.ReactNode }) {
  const [ideaGenState, setIdeaGenState] = useState<IdeaGenState>(INITIAL_IDEAGEN_STATE)
...
export const useIdeaGen = () => {
  ...
  if (!context) throw new Error('useIdeaGen must be used within IdeaGenProvider')
  return context
}
