...
export default function Sidebar() {
  ...
  const [editingDescriptionValue, setEditingDescriptionValue] = useState(sidebarDescription)
  const descriptionInputRef = useRef<HTMLInputElement>(null)

  // Drag and drop state
  const [draggedItem, setDraggedItem] = useState<string | null>(null)
  const [dragOverItem, setDragOverItem] = useState<string | null>(null)
  const [dragGroup, setDragGroup] = useState<'start' | 'learnResearch' | null>(null)
  ...
  const handleDragOver = (e: React.DragEvent, href: string, groupId: 'start' | 'learnResearch') => {
    e.preventDefault()
          ...
          <div key={group.id}>
            ...
            <div
              className={`text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 truncate transition-all duration-300 ${
                sidebarCollapsed ? 'opacity-0 h-0 overflow-hidden px-0' : 'opacity-100 px-1'
              }`}
            >
              {group.name}
            </div>
