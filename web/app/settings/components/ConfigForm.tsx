...
import { Plus, Check, Loader2, Eye, EyeOff, ChevronDown, TestTube } from 'lucide-react'
...
export default function ConfigForm({
  ...
  const initialUseEnvBaseUrl = editConfig ? isEnvReference(editConfig.base_url) : false
  const initialUseEnvApiKey = editConfig ? isEnvReference(editConfig.api_key) : false

  // Form state
  const [name, setName] = useState(editConfig?.name || '')
  const [provider, setProvider] = useState(editConfig?.provider || PROVIDER_OPTIONS[configType][0])
  const [baseUrl, setBaseUrl] = useState(editConfig ? getDisplayValue(editConfig.base_url) : '')
  const [useEnvBaseUrl, setUseEnvBaseUrl] = useState<boolean>(initialUseEnvBaseUrl)
  const [apiKey, setApiKey] = useState('')
  const [useEnvApiKey, setUseEnvApiKey] = useState<boolean>(initialUseEnvApiKey)
  ...
  const handleProviderChange = (newProvider: string) => {
    ...
    if (LOCAL_PROVIDERS.includes(newProvider) && LOCAL_PROVIDER_URLS[newProvider]) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/v1/config/${configType}/test`), {
        ...
        body: JSON.stringify({
          provider,
          base_url: useEnvBaseUrl ? { use_env: getEnvVarForBaseUrl(configType) } : baseUrl,
          api_key: isLocalProvider
            ...
            : useEnvApiKey
              ...
              : apiKey,
          model,
          ...(configType === 'embedding' ? { dimensions: Number(dimensions) } : {}),
      ...
      setTestResult({
        ...
        message:
          data.message || (data.success ? t('Connection successful') : t('Connection failed')),
      })
    } catch (e: any) {
      setTestResult({
        success: false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isSearchConfig) {
        payload.base_url = useEnvBaseUrl ? { use_env: getEnvVarForBaseUrl(configType) } : baseUrl
        payload.model = model
      }
      ...
      setError(isEditMode ? t('Failed to update configuration') : t('Failed to add configuration'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    ...
    >
      <h3 className="font-medium text-slate-900 dark:text-slate-100 mb-4">
        {isEditMode ? `${t('Edit Configuration')}: ${editConfig.name}` : t('Add New Configuration')}
      </h3>
