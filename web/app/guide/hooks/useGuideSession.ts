...
export function useGuideSession() {
  ...
  useEffect(() => {
    ...
    const persistedSession = loadFromStorage<Partial<SessionState>>(STORAGE_KEYS.GUIDE_SESSION, {})
    ...
    if (Object.keys(persistedSession).length > 0) {
      ...
      setSessionState(prev => mergeWithDefaults(persistedSession, prev, GUIDE_SESSION_EXCLUDE))
    }
  ...
  }, [sessionState.session_id, addLoadingMessage, removeLoadingMessage, addChatMessage])
    ...
    try {
      ...
      if (data.success) {
        if (data.status === 'completed') {
          ...
          addChatMessage(
            'system',
            data.message || '🎉 Congratulations on completing all knowledge points!',
            `complete-${Date.now()}`
          )
        } else {
          setSessionState(prev => ({
  ...
  const sendMessage = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        if (data.success) {
          addChatMessage('assistant', data.answer || '', `assistant-${Date.now()}`)
        } else {
          addChatMessage(
            'assistant',
    ...
    async (bugDescription: string) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          addChatMessage('system', '✅ HTML page has been fixed!', `fix-${Date.now()}`)
          return true
        } else {
          addChatMessage(
            'system',
        ...
        addChatMessage('system', '❌ Fix failed, please try again later', `error-${Date.now()}`)
        return false
      }
    },
    [sessionState.session_id, addLoadingMessage, removeLoadingMessage, addChatMessage]
