...
import { loadFromStorage, saveToStorage, persistState, mergeWithDefaults } from '@/lib/persistence'
...
export function useStatePersistence<T extends Record<string, any>>(
  ...
  const loadPersistedState = useCallback((): T => {
    ...
    const persisted = loadFromStorage<Partial<T>>(key, {})
    return mergeWithDefaults(persisted, defaultState, excludeFields as (keyof T)[])
