...
logger: Logger = get_logger("RosterGen")
...
def fetch_repo_stats(owner: str, repo: str, token: str | None = None) -> dict:
    """Fetch repository statistics (accurate counts) from GitHub API."""
    headers = {
        "Accept": "application/vnd.github.v3+json",
        "User-Agent": "Repo-Roster-Generator",
    }
    if token:
    ...
    try:
        ...
        logger.exception("Error fetching repo stats")
...
def fetch_avatar_as_base64(avatar_url: str, size: int = 48) -> str | None:
    ...
    try:
        ...
        logger.exception("Error fetching avatar %s", avatar_url)
        return None
...
def generate_modern_roster_svg(
    ...
    # Fetch avatars
    logger.info(f"Fetching {len(display_users)} avatars for {title}...")
    avatar_cache = {}
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(future_to_user):
            ...
            try:
                ...
                logger.exception("Error processing %s: %s", login, exc)
...
def main():
    ...
    logger.info(f"Fetching repo stats for {args.repo}...")
    ...
    logger.info(f"Fetched {len(stargazers)} stargazer records for avatars")

    # Fetch forkers (only need latest ones for avatars)
    logger.info(f"Fetching forkers for {args.repo}...")
    ...
    logger.info("Generating stargazers SVG...")
    ...
    logger.info(f"Saved: {stargazers_path}")

    # Generate forkers SVG
    logger.info("Generating forkers SVG...")
    ...
    with open(forkers_path, "w", encoding="utf-8") as f:
        f.write(forkers_svg)
    logger.info(f"Saved: {forkers_path}")
