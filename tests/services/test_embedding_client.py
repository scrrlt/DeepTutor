...
from src.services.embedding.config import EmbeddingConfig, _strip_value, _to_int, _to_bool
from src.services.embedding.adapters.base import EmbeddingRequest, EmbeddingResponse
...
def mock_adapter():
    adapter = MagicMock()
    adapter.embed = AsyncMock(return_value=EmbeddingResponse(
        embeddings=[[0.1, 0.2]],
        model="test-model",
        dimensions=2,
