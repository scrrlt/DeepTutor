...
def test_search_execution(provider: TavilyProvider) -> None:
    ...
    with patch("src.services.search.providers.tavily.httpx.post", return_value=mock_http_response):
...
def test_search_error_handling(provider: TavilyProvider) -> None:
    ...
    with (
        patch("src.services.search.providers.tavily.httpx.post", return_value=mock_http_response),
