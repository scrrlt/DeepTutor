...
class TestUnifiedConfigManager:
    ...
    def mock_settings_dir(self, tmp_path: Path):
        ...
        with patch(
            "src.services.config.unified_config.SETTINGS_DIR", tmp_path
        ):
            yield tmp_path
    ...
    def test_build_default_config_llm(
    ...
    def test_list_configs(
        ...
        data = {
            "configs": [
                {"id": "custom1", "name": "Custom 1", "provider": "anthropic"}
            ],
            "active_id": "custom1",
        }
    ...
    def test_delete_default_config_fail(
        self, config_manager: UnifiedConfigManager
    ):
    ...
    def test_set_active_config(self, config_manager: UnifiedConfigManager):
        ...
        added = config_manager.add_config(
            ConfigType.LLM, {"name": "Active Candidate"}
        )
        cid = added["id"]
    ...
    def test_get_active_llm_config_helper(
        self, config_manager: UnifiedConfigManager
    ):
