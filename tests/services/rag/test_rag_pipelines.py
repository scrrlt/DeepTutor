...
This module contains integration tests for RAG pipelines including RAGAnything,
LightRAG, LlamaIndex, and Academic.
...
# --- 1. PATH SETUP (Critical: Must be before project imports) ---
...
# Initialize Logger ONCE for the file
...
class TestRAGToolIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_available_providers(self):
        ...
        logger.info("Testing list_available_providers")
        ...
        for expected_id in expected:
            ...
            self.assertIn(
                expected_id, provider_ids, f"Missing provider: {expected_id}"
            )

        logger.success("Test list_available_providers passed")
    ...
    async def test_get_current_provider(self):
        ...
        self.assertIn(
            provider, ["raganything", "lightrag", "llamaindex", "academic"]
        )
    ...
    async def test_has_pipeline_valid(self):
        ...
        for name in ["raganything", "lightrag", "llamaindex", "academic"]:
            self.assertTrue(
                has_pipeline(name), f"Pipeline {name} should exist"
            )
...
class TestPipelineFactory(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_pipeline_raganything(self):
        ...
        logger.info(
            "✓ RAGAnything pipeline created: %s", type(pipeline).__name__
        )
    ...
    async def test_get_pipeline_lightrag(self):
        ...
        logger.info("✓ LightRAG pipeline created: %s", pipeline.name)
    ...
    async def test_get_pipeline_llamaindex(self):
        ...
        logger.info("✓ LlamaIndex pipeline created: %s", pipeline.name)
    ...
    async def test_get_pipeline_academic(self):
        ...
        logger.info("✓ Academic pipeline created: %s", pipeline.name)
    ...
    async def test_get_pipeline_invalid(self):
        ...
        logger.info("✓ Invalid pipeline correctly raises error")


class TestRAGAnythingPipeline(
    unittest.IsolatedAsyncioTestCase, RAGPipelineTestBase
):
    ...
    async def test_raganything_availability(self):
        ...
        logger.info("=== Testing RAGAnything Availability ===")
        ...
        try:
            ...
            logger.info("✓ RAGAnythingPipeline class available")
            ...
            try:
                ...
                logger.info("✓ RAG-Anything library available")
                return True
            except ImportError as e:
                logger.warning("⚠ RAG-Anything library not available: %s", e)
                logger.info(
                    "  Install RAG-Anything to enable full functionality"
                )
                return False


        except ImportError as e:
            logger.error("✗ Failed to import RAGAnythingPipeline: %s", e)
            return False
    ...
    async def test_raganything_full_workflow(self):
        ...
        logger.info("=== Testing RAGAnything Full Workflow ===")
        ...
        # Test initialization
        logger.info("Initializing KB with document: %s", self.test_doc)
        ...
        # Test search
        logger.info("Searching: %s", self.TEST_QUERY)
        ...
        logger.info("✓ Search result: %s...", result["answer"][:100])
...
class TestLlamaIndexPipeline(
    ...
    async def test_llamaindex_components(self):
        ...
        logger.info("=== Testing LlamaIndex Pipeline Components ===")
        ...
        logger.info("✓ Parser: %s", pipeline._parser.name)
        logger.info("✓ Chunkers: %s", [c.name for c in pipeline._chunkers])
        logger.info("✓ Embedder: %s", pipeline._embedder.name)
        logger.info("✓ Indexers: %s", [i.name for i in pipeline._indexers])
        logger.info("✓ Retriever: %s", pipeline._retriever.name)
    ...
    async def test_llamaindex_full_workflow(self):
        ...
        logger.info("=== Testing LlamaIndex Full Workflow ===")
        ...
        try:
            ...
            logger.warning(
                "⚠ Initialization failed (may need PDF support): %s", e
            )
            return
        ...
        logger.info("Searching: %s", self.TEST_QUERY)
        result = await pipeline.search(
            query=self.TEST_QUERY, kb_name=self.TEST_KB_NAME
        )
        self.assertIn("answer", result)
        logger.info("✓ Search result: %s...", result.get("answer", "")[:100])


class TestLightRAGPipeline(
    unittest.IsolatedAsyncioTestCase, RAGPipelineTestBase
):
    ...
    async def test_lightrag_components(self):
        ...
        logger.info("=== Testing LightRAG Pipeline Components ===")
        ...
        # Check components
        self.assertIsNotNone(pipeline._parser)
        # LightRAG may encapsulate chunking & embedding inside its indexer
        self.assertTrue(
            len(pipeline._chunkers) > 0 or len(pipeline._indexers) > 0
        )
        ...
        if pipeline._embedder:
            ...
            logger.info(
                "✓ Embedder: (handled by indexer) %s",
                [i.name for i in pipeline._indexers],
            )
...
class TestAcademicPipeline(
    ...
    async def test_academic_components(self):
        ...
        logger.info("=== Testing Academic Pipeline Components ===")
        ...
        self.assertIsNotNone(pipeline._parser)
        self.assertEqual(
            len(pipeline._chunkers), 2
        )  # SemanticChunker + NumberedItemExtractor
        self.assertIsNotNone(pipeline._embedder)
        self.assertGreater(len(pipeline._indexers), 0)
        self.assertIsNotNone(pipeline._retriever)
...
class TestRAGToolWithProviders(
    unittest.IsolatedAsyncioTestCase, RAGPipelineTestBase
):
    ...
    async def test_rag_search_invalid_provider(self):
        ...
        logger.info("=== Testing Invalid Provider Error ===")
        ...
        self.assertIn("Unknown pipeline", str(context.exception))
        logger.info(
            "✓ Invalid provider correctly raises error: %s", context.exception
        )
    ...
    async def test_rag_search_default_provider(self):
        ...
        # They should match
        logger.info("✓ Current provider: %s", current)
...
class TestComponentAvailability(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_parsers_available(self):
        ...
        logger.info("=== Testing Parser Availability ===")

        from src.services.rag.components.parsers import (
            MarkdownParser,
            PDFParser,
        )
    ...
    async def test_embedders_available(self):
        ...
        logger.info("✓ OpenAIEmbedder: %s", embedder.name)
    ...
    async def test_indexers_available(self):
        ...
        logger.info("=== Testing Indexer Availability ===")

        from src.services.rag.components.indexers import (
            GraphIndexer,
            VectorIndexer,
        )
        ...
        logger.info("✓ VectorIndexer: %s", vector.name)

        graph = GraphIndexer()
        print(f"✓ GraphIndexer: {graph.name}")

        logger.info("✓ GraphIndexer: %s", graph.name)
    ...
    async def test_retrievers_available(self):
        ...
        logger.info("=== Testing Retriever Availability ===")

        from src.services.rag.components.retrievers import (
            DenseRetriever,
            HybridRetriever,
        )
        ...
        logger.info("✓ DenseRetriever: %s", dense.name)

        hybrid = HybridRetriever()
        logger.info("✓ HybridRetriever: %s", hybrid.name)
...
class TestExistingKnowledgeBase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_existing_kb(self):
        ...
        logger.info("=== Testing Existing Knowledge Base ===")
        ...
        for mode in ["naive", "hybrid"]:
            ...
            try:
                ...
                logger.info(
                    "✓ Provider: %s", result.get("provider", "unknown")
                )
                logger.info(
                    "✓ Answer length: %s chars", len(result.get("answer", ""))
                )
                logger.info("✓ Preview: %s...", result.get("answer", "")[:200])
            except Exception:
                logger.exception("✗ Failed to search existing KB")
...
def run_quick_tests():
    ...
    logger.info("=" * 60)
    logger.info("Running Quick RAG Pipeline Tests")
    logger.info("=" * 60)
...
def run_all_tests():
    ...
    logger.info("=" * 60)
    logger.info("Running All RAG Pipeline Tests")
    logger.info("=" * 60)
    logger.info("Environment variables for full testing:")
    logger.info("  TEST_RAG_ANYTHING=true  - Run RAGAnything tests")
    logger.info("  TEST_LLAMAINDEX=true    - Run LlamaIndex tests")
    logger.info("  TEST_ALL_PIPELINES=true - Run all pipeline tests")
    logger.info("  TEST_EXISTING_KB=true   - Test existing KBs")
    logger.info("  TEST_KB_NAME=<name>     - KB name for existing KB test")
    logger.info("  TEST_QUERY=<query>      - Query for existing KB test")
    logger.info("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run only quick tests"
    )
