...
async def test_rag_service_initialize(mock_get_pipeline, rag_service: RAGService):
    ...
    mock_pipeline.initialize.assert_called_once_with(kb_name="test_kb", file_paths=["file1.txt", "file2.txt"])
...
async def test_rag_service_search(mock_get_pipeline, rag_service: RAGService):
    ...
    with patch.object(rag_service, '_get_provider_for_kb', return_value="test_provider"):
        ...
        mock_pipeline.search.assert_called_once_with(query="test_query", kb_name="test_kb", mode="hybrid")
...
async def test_rag_service_delete_manual(mock_path_exists, mock_rmtree, mock_get_pipeline, rag_service: RAGService):
    """
