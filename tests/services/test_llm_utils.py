...
def test_is_local_llm_server():
    ...
    assert utils.is_local_llm_server("http://host.docker.internal:8000") # Common dev setup might be considered local effectively or caught by port logic if port matches
...
def test_sanitize_url():
    ...
    assert utils.sanitize_url("localhost:11434/api/chat") == "http://localhost:11434/api/chat"
    # But it adds http
...
def test_clean_thinking_tags():
    ...
    assert utils.clean_thinking_tags(None) is None
...
def test_build_chat_url():
    ...
    # Already has suffix
    assert utils.build_chat_url("http://host/v1/chat/completions") == "http://host/v1/chat/completions"
    
    # Azure
    assert "api-version=2023-05-15" in utils.build_chat_url("https://azure", api_version="2023-05-15")
