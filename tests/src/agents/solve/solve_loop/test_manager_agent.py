...
from src.agents.solve.memory import InvestigateMemory, SolveMemory, KnowledgeItem
...
async def test_manager_agent_process_workflow(manager_agent: ManagerAgent):
    ...
    """
    manager_agent.call_llm = AsyncMock(return_value='{"steps": [{"step_id": "S1", "target": "t1"}]}')
...
@pytest.mark.asyncio
async def test_manager_agent_process_existing_steps(manager_agent: ManagerAgent):
    """
