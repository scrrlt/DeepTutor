...
def precision_answer_agent():
    ...
    with patch('src.agents.base_agent.get_prompt_manager'), \
        ...
        agent = PrecisionAnswerAgent(config=config, api_key="test_key", base_url="http://localhost:1234")
        yield agent


@pytest.mark.asyncio
async def test_precision_answer_agent_process_workflow(precision_answer_agent: PrecisionAnswerAgent):
    """
...
@pytest.mark.asyncio
async def test_generate_precision_answer(precision_answer_agent: PrecisionAnswerAgent):
    """
    ...
    result = await precision_answer_agent._generate_precision_answer("question", "detailed_answer", True)
