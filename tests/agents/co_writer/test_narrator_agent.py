...
def narrator_agent():
    ...
    with patch('src.agents.base_agent.get_prompt_manager'), \
         ...
         patch('src.agents.co_writer.narrator_agent.get_tts_config', return_value={"model": "tts-1", "api_key": "test_key", "base_url": "http://localhost:1234"}):
        agent = NarratorAgent()
        yield agent
...
async def test_narrator_agent_generate_script(narrator_agent: NarratorAgent):
    """
    Tests that the generate_script method correctly calls the LLM and returns the script.
    """
...
async def test_narrator_agent_generate_audio(mock_openai, narrator_agent: NarratorAgent):
    """
    Tests that the generate_audio method correctly calls the TTS API and saves the audio file.
    """
...
async def test_narrator_agent_narrate_workflow(narrator_agent: NarratorAgent):
    """
    Tests the overall workflow of the narrate method.
    """
    narrator_agent.generate_script = AsyncMock(return_value={"script": "s", "key_points": [], "original_length": 1, "script_length": 1})
    narrator_agent.generate_audio = AsyncMock(return_value={"audio_path": "path", "audio_url": "url", "audio_id": "id"})
...
async def test_extract_key_points(narrator_agent: NarratorAgent):
    """
    Tests the _extract_key_points method.
    """
...
def test_validate_tts_config(narrator_agent: NarratorAgent):
    """
    Tests the _validate_tts_config method.
    """
