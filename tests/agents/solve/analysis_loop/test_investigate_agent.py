...
@pytest.mark.asyncio
async def test_investigate_agent_process_workflow(
...
@pytest.mark.asyncio
async def test_investigate_agent_process_empty_plan(
    investigate_agent: InvestigateAgent,
):
...
def test_build_system_prompt_web_search_disabled(
    investigate_agent: InvestigateAgent,
):
    ...
    assert (
        "`web_search`" not in prompt
        or "This is a system prompt with Web search is disabled.." in prompt
    )
