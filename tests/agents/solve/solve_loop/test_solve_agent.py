...
async def test_solve_agent_process_workflow(solve_agent: SolveAgent):
    ...
    """
    solve_agent.call_llm = AsyncMock(
        return_value='{"tool_calls": [{"type": "rag_hybrid", "intent": "test query"}]}'
    )
    ...
    result = await solve_agent.process(
        "question", step, solve_memory, investigate_memory, citation_memory
    )
