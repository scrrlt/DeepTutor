...
async def test_main_solver_default_initialization():
    ...
    """
    with patch('src.agents.solve.main_solver.load_config_with_main_async', new_callable=AsyncMock) as mock_load_config:
        ...
        with patch('src.services.llm.config.get_llm_config_async', new_callable=AsyncMock) as mock_get_llm_config:
            mock_get_llm_config.return_value = MagicMock(api_key="test_api_key", base_url="http://localhost:1234")
...
async def test_main_solver_ainit_missing_api_key():
    ...
    with patch('src.agents.solve.main_solver.load_config_with_main_async', new_callable=AsyncMock) as mock_load_config:
        ...
        with patch('src.services.llm.config.get_llm_config_async', new_callable=AsyncMock) as mock_get_llm_config:
            mock_get_llm_config.return_value = MagicMock(api_key=None, base_url="https://api.openai.com/v1")
...
async def test_main_solver_solve_workflow():
    ...
    with patch('src.agents.solve.main_solver.load_config_with_main_async', new_callable=AsyncMock) as mock_load_config:
        ...
        with patch('src.services.llm.config.get_llm_config_async', new_callable=AsyncMock) as mock_get_llm_config:
            ...
            with patch.object(solver, '_run_dual_loop_pipeline', new_callable=AsyncMock) as mock_run_pipeline:
...
async def test_main_solver_dual_loop_pipeline():
    ...
    with patch('src.services.llm.config.get_llm_config_async', new_callable=AsyncMock) as mock_get_llm_config:
        ...
        solver.investigate_agent.process.return_value = {"should_stop": True, "knowledge_item_ids": ["1"]}
        solver.note_agent.process.return_value = {"success": True, "processed_items": 1}
        ...
        solver.precision_answer_agent.process.return_value = {"needs_precision": True, "precision_answer": "precise answer"}

        # Mock memory objects
        with patch('src.agents.solve.main_solver.InvestigateMemory') as mock_investigate_memory, \
