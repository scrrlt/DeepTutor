...
@pytest.fixture
def guide_manager(tmp_path):
    """
    Provides a GuideManager instance with mocked dependencies.
    """
    with patch('src.agents.guide.guide_manager.load_config_with_main', return_value={"system": {"language": "en"}, "paths": {"user_log_dir": "/tmp/logs"}}):
        manager = GuideManager(api_key="test_key", base_url="http://localhost:1234", output_dir=str(tmp_path))
        yield manager
...
async def test_create_session(guide_manager: GuideManager):
    ...
    """
    guide_manager.locate_agent.process = AsyncMock(return_value={"success": True, "knowledge_points": [{"title": "kp1"}]})
...
async def test_start_learning(guide_manager: GuideManager):
    """
    Tests that the start_learning method correctly starts the learning process.
    """
    session = MagicMock()
...
async def test_next_knowledge(guide_manager: GuideManager):
    """
    Tests that the next_knowledge method correctly moves to the next knowledge point.
    """
    session = MagicMock()
...
async def test_chat(guide_manager: GuideManager):
    """
    Tests that the chat method correctly handles chat interactions.
    """
    session = MagicMock()
...
async def test_fix_html(guide_manager: GuideManager):
    """
    Tests that the fix_html method correctly fixes HTML page bugs.
    """
    session = MagicMock()
    session.knowledge_points = [{"title": "kp1"}]
    session.current_index = 0
    guide_manager._load_session = MagicMock(return_value=session)
    guide_manager.interactive_agent.process = AsyncMock(return_value={"success": True, "html": "<html>"})
...
def test_save_and_load_session(guide_manager: GuideManager, tmp_path):
    """
    Tests that sessions are saved and loaded correctly.
    """
    guide_manager.locate_agent.process = AsyncMock(return_value={"success": True, "knowledge_points": [{"title": "kp1"}]})
    
    # Can't use await in a non-async test, so we run the async function to completion
    result = asyncio.run(guide_manager.create_session("nid", "nname", []))
